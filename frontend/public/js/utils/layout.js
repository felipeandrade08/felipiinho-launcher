...
(function carregarVisualRealista() {
  ...
  link.rel = 'stylesheet';
  link.href = 'css/realistic-ui.css';
  link.dataset.realisticUi = 'true';
...
function renderizarSidebar(paginaAtiva){
  const grupos=obterMenuAtual().map(grupo=>`
    ...
    ${grupo.itens.map(item=>`<a href="${item.href}" class="sidebar-link ${item.id===paginaAtiva?'ativo':''}" data-badge="${item.badge||''}">
      <i class="fa-solid ${item.icone}"></i><span>${item.label}</span>
...
function renderizarTopbar(titulo,subtitulo){
  ...
  const iniciais=(usuario.nome||'?').trim().split(' ').filter(Boolean).slice(0,2).map(p=>p[0]).join('').toUpperCase();
  const cargo=AuthService.obterCargoBadge();
  const coresMap={Administrador:'var(--verde-limao)',Diretoria:'#FFB020',RH:'#5865F2',Motorista:'var(--cinza-400)'};
  const corCargo=coresMap[cargo]||'var(--cinza-400)';
  return `<header class="topbar"><div class="d-flex align-items-center gap-3">
    <button class="btn-toggle-sidebar" id="btnToggleSidebar" aria-label="Abrir menu"><i class="fa-solid fa-bars"></i></button>
    <div class="topbar-titulo"><h1>${titulo}</h1><p>${subtitulo||''}</p></div></div>
    <div class="topbar-acoes"><div class="busca-topbar d-none d-md-block"><i class="fa-solid fa-magnifying-glass"></i><input type="text" placeholder="Buscar no sistema..." /></div>
    <div class="icone-notificacao" id="btnNotificacoes"><i class="fa-regular fa-bell"></i><span class="bolha" id="bolhaNotificacao" style="display:none;"></span></div>
    <div class="perfil-usuario" id="btnPerfilUsuario"><div class="perfil-usuario__avatar">${iniciais}</div><div><div class="perfil-usuario__nome">${usuario.nome}</div><div class="perfil-usuario__cargo" style="color:${corCargo};font-weight:600;">${cargo}</div></div><i class="fa-solid fa-chevron-down" style="font-size:.7rem;color:var(--cinza-600)"></i></div></div>
  </header>`;
}
...
function montarLayout({paginaAtiva,titulo,subtitulo}){
  ...
  raiz.innerHTML=`${renderizarSidebar(paginaAtiva)}<div class="conteudo-principal">${renderizarTopbar(titulo,subtitulo)}<main class="area-pagina" id="conteudoPagina">${htmlConteudo}</main></div>`;
  const btnToggle=document.getElementById('btnToggleSidebar'),sidebar=document.getElementById('sidebarPrincipal'),overlay=document.getElementById('overlaySidebar');
  ...
  btnToggle?.addEventListener('click',abrirMenu);overlay?.addEventListener('click',fecharMenu);
  document.getElementById('btnPerfilUsuario')?.addEventListener('click',()=>{
    Swal.fire({title:'Sair do sistema?',text:'Você precisará fazer login novamente para voltar a acessar o FELIPINHO LAUNCHER.',icon:'question',showCancelButton:true,confirmButtonText:'Sair',cancelButtonText:'Cancelar',confirmButtonColor:'#0B0B0B',cancelButtonColor:'#9AA39E'}).then(resultado=>{
      if(resultado.isConfirmed){AuthService.encerrarSessao();window.location.href='index.html';}
    });
  });
...
(function initNotificacoes(){
  ...
  function atualizarBadge(total){const badge=document.getElementById('badgePendentes');if(!badge)return;if(total>0){badge.textContent=total;badge.style.display='';}else badge.style.display='none';const titulo=document.title.replace(/^\(\d+\) /,'');document.title=total>0?`(${total}) ${titulo}`:titulo;}
  try{const token=AuthService.obterToken?AuthService.obterToken():sessionStorage.getItem('gr_token');const apiBase=window.GR_API_URL||'http://localhost:3000/api';const es=new EventSource(apiBase+'/notificacoes/stream'+(token?'?token='+encodeURIComponent(token):''));es.addEventListener('pendentes_atualizados',e=>{try{atualizarBadge(JSON.parse(e.data).total);}catch(_){}});es.onerror=()=>{es.close();iniciarPolling();};}catch(_){iniciarPolling();}
  function iniciarPolling(){async function poll(){try{const d=await ApiService.get('/notificacoes/pendentes');atualizarBadge(d.total||0);}catch(_){} }poll();setInterval(poll,30000);}
})();
